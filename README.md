...
Программа для автоматического распознавания речи из аудиофайлов с использованием библиотеки [Vosk](https://alphacephei.com/vosk).

## Возможности

- ✅ Поддержка множества аудиоформатов (WAV, MP3, OGG, FLAC, M4A, WMA, AAC)
- ✅ Автоматическая конвертация в нужный формат через ffmpeg
- ✅ Пакетная обработка файлов из директории
- ✅ Сохранение результатов в формате Markdown
- ✅ Автоматическое определение языка из названия модели
...
- Python 3.7 или выше
- ffmpeg (устанавливается отдельно, см. [инструкцию ниже](#3-установка-ffmpeg))
- Модель Vosk для нужного языка

**Примечание:** `ffmpeg` - это системная программа (не Python-библиотека), поэтому её нужно установить отдельно. Программа автоматически подскажет как это сделать при первом запуске, если ffmpeg не установлен.
...
Отредактируйте файл `config.json` - **параметр `language` не обязателен**:
...
Язык будет **автоматически определен** из названия модели! 🎉
...
**Параметры:**
- `model_path` - путь к распакованной модели Vosk. **Именно модель определяет язык распознавания!**
- `input_dir` - директория с входными аудиофайлами
- `output_dir` - директория для сохранения результатов транскрипции
- `processed_dir` - директория для обработанных аудиофайлов (перемещаются после успешной обработки)
- `sample_rate` - частота дискретизации (обычно 16000 Hz)
- `supported_formats` - список поддерживаемых форматов файлов
- `language` - **[ОПЦИОНАЛЬНО]** код языка (ru, en, de, fr и т.д.)
  - **Можно вообще не указывать** - язык определится автоматически из названия модели
  - Нужен только для моделей с нестандартными названиями
  - При несовпадении с моделью автоматически используется язык модели

**✨ Автоопределение языка:** Программа автоматически извлекает язык из названия модели (например, `vosk-model-small-ru-0.22` → `ru`). Параметр `language` больше не обязателен! Это упрощает конфигурацию и исключает ошибки.
